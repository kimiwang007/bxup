...
<body>
	<div class="container" style="width:100%;">
        ...
        <div class="col-xs-12">        
			<input type="button"  class="btn btn-primary" value="新建社区" onclick="javascript:window.location.href='./subscribeAdd/community'"/>
						...
						<tr>
							...
							<td>
								<c:if test="${item.shelves == -1}">
									<input type="button"  class="btn btn-primary" onclick="javascript:window.location.href='community/community_shelves/${item.id}'" value="上架">
								</c:if>
								<c:if test="${item.shelves == 1}">
									<input type="button"  class="btn btn-danger" onclick="javascript:window.location.href='community/community_shelves/${item.id}'" value="下架">
								</c:if>
							</td>
							<td>
								<input type="button"  class="btn btn-primary" onclick="javascript:window.location.href='community/community_edit/${item.id}'" value="编辑">
    ...
    function delConfirm(itemId){	
    	if(window.confirm('确定要删除该活动吗?')){
    		javascript:window.location.href='community/community_delete/' + itemId
        }
    }
    function setFeedIMG(){
    	...
    	javascript:window.location.href='./community/' + chk_value;
    }
    function back(){
